...
class FS_WC_Wallet {
    ...
    public function wallet_topup() {
        ...
        if (isset($_POST['fsww_balance_to_add']) && !empty($_POST['fsww_balance_to_add'])) {
            ...
            if ($is_valid['is_valid']) {
                add_filter('woocommerce_add_cart_item_data', array($this, 'add_fsww_product_price_to_cart_item_data'),
                    10, 2);
    ...
    function process_withdrawal_request() {
        ...
        if ($amount < $min_amount || $min_amount > $max_amount) {
            $errors[] = 'amount=1';
        }
    ...
    public function fsww_i_withdraw_request() {
        ...
        if ($notify == 'on') {
            ...
            $message = '<p>' . fsww_price($amount) . ' ' . __('Have been withdrawed from your account balance',
                    'fsww') . '</p><br>' . $message;
    ...
    public function fsww_i_lock_request() {
        ...
        if ($action == 'lock') {
            ...
            if ($notify == 'on') {
                ...
                $message = '<p>' . __('Your wallet balance have been locked, you can nolonger use it to pay for products.',
                        'fsww') . '</p><br>' . $message;
    ...
    public function add_actions() {
        ...
        add_filter('woocommerce_product_categories_widget_args', array($this, 'exclude_product_categories_widget'), 10,
            1);
        ...
        add_action('woocommerce_ajax_save_product_variations', array($this, 'woo_add_custom_variable_fields_save_ajax'),
            10, 2);
        ...
        <div class="fsww-meke-deposit-sc">
            ...
            <form id="send_money" method="post">
				...
				<p class="form-row form-row-wide">
					...
					<input type="text" class="input-text fsww-input woocommerce-Input woocommerce-Input--text input-text" name="amount" id="amount" placeholder="0.00" value="" required>
        ...
        $input = ' <form method="post" action="">
                    <div class="woo-wallet-add-amount">
                        ...
                        <p class="woocommerce-form-row woocommerce-form-row--wide form-row form-row-wide">
                             ...
                             <input type="number" step="0.01" value="' . $min . '" min="' . $min . '"  max="' . $max . '" name="fsww_balance_to_add" id="fsww_balance_to_add" class="input-text fsww-input woocommerce-Input woocommerce-Input--text input-text" />
    ...
    public function account_alerts() {
        ...
        if ($status['status'] == 'locked') {
            ...
            $notices .= __('Your wallet credit is locked for the following reason:',
                    'fsww') . '<br><b>' . $status['lock_message'] . '<br>' . __('If you think there is an error please contact us.',
                    'fsww') . '</b>';
            $show = true;
    ...
    public function add_filters() {
        ...
        if (get_option('fsww_refunds', 'on') == 'on') {
            ...
            if (get_option('fsww_refunds_button_position', 'table') == 'table') {
                add_filter('woocommerce_my_account_my_orders_actions', array($this, 'add_refund_request_button'), 100,
                    2);
            }
